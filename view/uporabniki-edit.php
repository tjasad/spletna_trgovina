...
<div>
    ...
    <form action="<?= BASE_URL . "seminarska_naloga/uredi_profil" ?>" method="post">
        ...
        <div>
            <input type="hidden" name="id" value="<?= htmlspecialchars($user["costumer_id"]) ?>"/>
            <label for="ime"><b>Ime</b></label>
            <input type="text" placeholder="Vnesite ime" name="name" id="ime" value="<?= htmlspecialchars($user["name"]) ?>"
                   required><br/>
            <label for="priimek"><b>Priimek</b></label>
            <input type="text" placeholder="Vnesite priimek" name="surname" id="priimek" value="<?= htmlspecialchars($user["surname"]) ?>"
                   required><br/>
            <label for="naslov"><b>Naslov</b></label>
            <input type="text" placeholder="Vnesite ulico" name="street" id="naslov" value="<?= htmlspecialchars($user["street"]) ?>"
                   required>
            <input type="text" placeholder="Vnesite hišno številko" name="house_number"
                   value="<?= htmlspecialchars($user["house_number"]) ?>" required>
            <input type="text" placeholder="Vnesite pošto" name="post" value="<?= htmlspecialchars($user["post"]) ?>" required>
            <input type="text" placeholder="Vnesite poštno številko" name="post_number"
                   value="<?= htmlspecialchars($user["post_number"]) ?>"/><br/>
            ...
            <?php
            $aktiven = htmlspecialchars($user["status"]);
            if ($aktiven ==1){
            ?>
                 <select name="status">
